...
| **Sesión** | An occurrence of a node with lifecycle `planificada → abierta → cerrada \| cancelada`. Attendance belongs to the session and rolls up the tree. Materialized from the node's schedule; a `cancelada` session leaves the compliance denominator. See ADR-0015. | a one-shot "closed" row; conflating a session with a whole edition |
| **Programación** | The node's recurring schedule (`programs.config.programacion`: weekday + start/end time) that materializes planned sessions between its dates. | ad-hoc per-month session creation |
| **Enlace de sesión** | The per-session magic link: an HMAC bearer token (only its hash stored), expiry-bounded + revocable, that lets a teacher with no account mark attendance / upload the plan / close from their phone. Public + token-gated; roster is nombre+apellidos only. See ADR-0015. | a permanent share link; a login |
| **Cierre de sesión** | Closing a session with its config-driven data (tema + `session_close_config` fields + required uploads). The generic cierre owns `session_close_config`; Familia reparto keeps its own preset. | the Familia albarán/hoja-firmas close (different subsystem) |
| **Responsable (sesión)** | Who ran/closed the session (`responsable_nombre`/`responsable_person_id`, or `en_nombre_de` when staff act for the teacher). The metric measures the session, never who typed. | attributing compliance to the data-entry user |
| **Documento de sesión** | A `session_documents` row in the private `program-documents` bucket (e.g. "Plan de la clase") — uploaded as a file OR a photo→OCR organized-text plan, satisfiable **before** closing. | the identity-document OCR (persons); a public inline URL |
...
| **High-risk fields** | `situacion_legal`, `recorrido_migratorio`. Read access restricted to superadmin/admin. `foto_documento_url` is narrower: superadmin-only, minted on demand and audited (ADR-0017). | — |
