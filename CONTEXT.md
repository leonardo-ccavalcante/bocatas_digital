...
| **Enlace de sesión** | The per-session magic link: an HMAC bearer token (only its hash stored), expiry-bounded + revocable, that lets a teacher with no account mark attendance / upload the plan / close from their phone. Public + token-gated; roster is nombre+apellidos only. See ADR-0014. | a permanent share link; a login |
