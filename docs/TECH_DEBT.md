...
| ID | Sev | ✓ | Title | Location | Status |
|----|-----|---|-------|----------|--------|
| **T-01** | P2 | ✅ | **CI runs integration tests with no live DB** (placeholder `http://localhost`, no `services: postgres`, no `supabase start`). ~41 tests can never pass in CI. Either wire an ephemeral Postgres/Supabase into the test job, or gate all DB tests behind a consistent `skipIf(!hasLiveDb)` so CI is honestly green. | `.github/workflows/ci.yml:56-63` | ☐ **open — CONFIRMED** 2026-06-11 Mythos (DIO-04, verified): placeholder localhost stands; ~79 tests + 13 files self-skip invisibly in CI. **→ 🔶 RED half fixed by PR #85 (2026-06-11):** the 11 docx/pdf tests that *hung* on dead `localhost` until the 5s timeout now mock the storage download (fixture v3) and run in ~100ms; the suite executes green in CI. The false-green half (the ~79 invisibly-skipped DB integration tests) **remains open** — wave tracked in #74. |
...
| **S-06** | P0 | ✅ | **Reparto tables directly CRUD-able via PostgREST by any self-signed-up user.** `delivery_rounds` + `delivery_round_assignments` carry `FOR ALL TO authenticated USING(true) WITH CHECK(true)`; combined with the blanket `authenticated` table grant (`20260612000002`) + GoTrue self-signup enabled, anyone can self-register → `authenticated` JWT → `/rest/v1/*` → read/tamper/delete the reparto schedule, bypassing the `adminProcedure` tRPC guard. `delivery_round_slots` carries the same policy (`20260707000001`), exposed once it deploys. | `supabase/migrations/20260521090001*`, `20260521090002*`, `20260707000001*` | 🔧 **CONFIRMED LIVE in prod 2026-07-07** (Supabase advisor `rls_policy_always_true` on both tables; `has_table_privilege('authenticated',…)` = full DML; `GET /auth/v1/settings` → `disable_signup:false`). **FIX WRITTEN** — migration `20260707000006_harden_reparto_rls.sql` (drops permissive policies + `REVOKE ALL FROM anon, authenticated`; service-role app unaffected — verified). `/security-review` PASS. **Pending:** (1) deploy migration; (2) disable GoTrue signup in prod dashboard (real users use Manus OAuth — only 4 vestigial `auth.users`, last login 2026-04-11). Relates to issue #50. |
| **S-07** | P3 | ✅ | **`autorizado_documento_url` returned to any authenticated user.** `families.compliance.verifyIdentity` is `protectedProcedure` (any authed) and returns it; `families.getById` (`voluntario`) `.select("*")` doesn't redact it; client renders a raw `<a href>`, not the role-gated `/manus-storage` proxy. | `server/routers/families/compliance.ts:80,118`; `families/crud.ts` getById; `client/.../IdentityVerifier.tsx:122` | ☐ **LATENT — NO live exposure (verified 2026-07-07 via MCP):** field is `NULL` for **all 604 families** (0 populated) and **every storage bucket is private** (`documentos-identidad` incl.), so a leaked URL opens nothing. Fix before the feature is populated: gate the field to admin/superadmin + serve via signed/proxied URL. **Infra side-note:** server code references a `family-documents` bucket (`rounds-ocr.ts:35`, `rounds-documents.ts:46`) that **does not exist in prod** (9 buckets, none named that) → reparto OCR/doc uploads fail on deploy unless a migration creates it. |
...
| **DIO-02** | P0 | 🔶 | **`main` HEAD CI is RED yet code landed** — lint failure aborts the suite before tests run (xref D-15/SIS-01). | `.github/workflows/ci.yml:46` | **✅ fixed by PR #85** (2026-06-11): lint exit 0 + full suite now executes; the "Lint, Typecheck & Test" gate passes on main (run 27339354255). Branch protection (DIO-01) still absent — gate is green but not *required*. |
