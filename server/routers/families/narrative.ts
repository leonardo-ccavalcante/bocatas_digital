...
export const narrativeRouter = router({
  ...
  composeNarrativeDraft: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // document. We pass the structured context (family composition, origin,
      // district) so the LLM does NOT repeat information already generated by
      // the compositor — preventing text duplication in the informe social.
      // Falls back to null (omit block) on LLM error.
      const observacionesRevisadas = await reviewObservaciones(
        t?.observaciones ?? null,
        {
          num_adultos: family.num_adultos,
          num_menores: family.num_menores_18,
