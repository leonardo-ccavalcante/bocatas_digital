...
export function offlineAttendanceRows(enriched: EnrichedOfflineItem[]) {
  return enriched.map(({ item, checkedInAt, checkedInDate }) => ({
 ...
 * DB actually inserted. Anonymous (person_id null) rows bypass the unique
 * arbiter (NULL <> NULL) so they are always "synced". The key uses the
 * per-item derived date so it matches what the upsert wrote.
 */
