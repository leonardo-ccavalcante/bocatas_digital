 ...
 *
 * Logic: informe social se renueva anualmente (INFORME_SOCIAL_RENEWAL_DAYS = 330 from compliance.ts).
 * "Renueva en los próximos daysAhead días" means:
 *   informe_social_fecha + 330 days <= today + daysAhead
 *   ↔ informe_social_fecha <= (today + daysAhead) - 330 days
 *
...
export const informesPorRenovarRouter = router({
  informesPorRenovar: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      maxInformeDate.setDate(
        maxInformeDate.getDate() + input.daysAhead - INFORME_SOCIAL_RENEWAL_DAYS,
      );
