/**
 * InformesPorRenovarModal.tsx — Familias cuyo informe social vence próximamente.
 *
...
import { useInformesPorRenovar } from "../hooks/useTemplatedReports";
...
export function InformesPorRenovarModal({ open, onClose }: Props) {
  const [daysAhead, setDaysAhead] = useState(30);

  const { data, isLoading, error } = useInformesPorRenovar({ daysAhead }, open);
  ...
  const rows = ((data?.rows ?? []) as unknown[]).map((r) => r as InformeRow);
      ...
      <DialogContent className="max-w-3xl">
        ...
        <div className="flex items-end gap-3 text-sm">
          <div className="space-y-1 w-40">
            <Label htmlFor="inf-days-ahead">Próximos días</Label>
            <Input
              id="inf-days-ahead"
              type="number"
              min={1}
        ...
        {!isLoading && !error && rows.length > 0 && (
          <div className="max-h-72 overflow-auto rounded-md border">
            <Table aria-label="Informes por renovar">
              <TableHeader>
                <TableRow>
                  <TableHead scope="col">#</TableHead>
                  <TableHead scope="col">Titular</TableHead>
                  <TableHead scope="col">Distrito</TableHead>
                  <TableHead scope="col">Fecha informe</TableHead>
        ...
        {!isLoading && !error && rows.length === 0 && data && (
          <p className="text-sm text-muted-foreground text-center py-4">
            Sin informes próximos a renovar en los próximos {daysAhead} días.
          </p>
        )}
      </DialogContent>
    </Dialog>
