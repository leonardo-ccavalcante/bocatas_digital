...
export function ListadoInternoPrint({ roundId, day, turno }: Props) {
  const { data, isLoading } = useListadoInterno(roundId, day, turno);
    ...
    <div className="space-y-3">
      ...
      <div id="listado-interno" className="rounded-lg border p-4 text-xs">
        <h2 className="mb-2 text-sm font-bold">
          {data.nombre ?? "Listado interno"} — {data.fecha_cita}
          {data.turno ? ` · ${TURNO_LABEL[data.turno] ?? data.turno}` : ""}
        </h2>
