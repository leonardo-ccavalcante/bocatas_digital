-- Actor-derivation fix for the INFORMES enrich RPC (same root cause as the
-- confirm RPC in 20260605000001): ctx.user.id is the Manus openId (non-UUID),
-- so `auth.uid()::text` (which casts JWT sub → uuid) raised 22P02 at runtime
-- for every real user. Derive the actor from `auth.jwt() ->> 'sub'` (no cast),
-- restoring the 20260506000003 pattern. Pure CREATE OR REPLACE — body, security
-- posture, savepoints, audit, sanitiser, grants are otherwise unchanged.
...
BEGIN
  ...
  FOR i IN 0 .. jsonb_array_length(v_families) - 1 LOOP
    ...
    BEGIN
      ...
      -- Member backfill — STRONG tiers only. Weak/none/ambiguous/conflict skipped.
